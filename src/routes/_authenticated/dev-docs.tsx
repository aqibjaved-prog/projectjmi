...
const MODULES: Array<{ name: string; status: "Complete" | "In Progress" | "Planned"; notes: string }> = [
  ...
  { name: "Vehicle Management", status: "Complete", notes: "Full fleet CRUD, photo upload, insurance/fitness/pollution expiry tracking, import/export, super-admin read-only cross-tenant view. Backend-enforced seating capacity: a Postgres trigger (students_check_vehicle_capacity) rejects INSERT/UPDATE that would exceed a vehicle's capacity; the vehicle_occupancy view (security_invoker) exposes capacity/occupied/available seats per vehicle and powers frontend badges, disabled selects, and dashboard totals. ASSIGNMENT RELATIONSHIPS (no new tables — reuses existing FKs): src/lib/vehicles.ts → fetchVehicleAssignments(schoolId) resolves each vehicle's assigned driver (from routes.vehicle_id → routes.driver_id, with drivers.assigned_vehicle_id as fallback), assigned route (routes.vehicle_id), today's trip and any currently-active trip (trips.vehicle_id filtered by trip_date=today OR status IN in_progress/paused). Vehicle list surfaces columns for Driver, Route, Today's Trip, Students N/capacity, and Availability (available / in use / maintenance / inactive via vehicleAvailability()). Vehicle detail page shows an Assignments card with driver + phone, route, active trip, capacity + utilisation bar (green <80% / amber ≥80% / red ≥100%), plus quick-action buttons View Driver / View Route / View Trip. Empty states render 'No driver assigned' / 'No route assigned' / 'No active trip'. Both queries auto-refresh (list 20s, detail 15s) and are invalidated on any mutation of vehicles/trips. Double-booking is prevented by assertVehicleAvailableForTrip(vehicleId, excludeTripId) — called from both trip-start sites (School Admin trips.$tripId startTrip and Driver Portal driver.today start) — which raises 'This vehicle is already on an active trip (…)' if any other trip on the same vehicle is in_progress or paused. RLS and multi-school isolation preserved: assignment queries scope by school_id and the caller can only read vehicles/routes/trips/drivers permitted by existing policies." },
  ...
  { name: "Driver Portal", status: "Complete", notes: "Fully separated /driver/* module. Reads via existing RLS (drivers.user_id = auth.uid()): Trips tenant read + Trips driver/admin write allow drivers to list/start/pause/resume/end their own trips only; Students tenant read exposes only students on the driver's assigned route; Vehicles/Routes filtered via join. Every driver page (Dashboard, Today's Trip, Navigation, Students, QR Scanner, Trip History, Live Trip) consumes the SAME shared source in src/lib/driver-portal.ts (useMyDriver / useDriverTrips / useDriverTripHistory / useDriverStudents / useDriverTrip / useDriverNotifications) — no duplicate queries. Trip assignment resolves through TWO paths: (a) trips.driver_id set directly, or (b) trips.driver_id IS NULL and the driver owns the route via routes.driver_id. Both paths are enforced in the trips RLS SELECT/ALL policies AND mirrored in the query .or() filter (driver_id.eq.<id>,and(driver_id.is.null,route_id.in.(...))), so a driver sees the trip even when the school admin only assigned them at the route level. Dashboard shows Vehicle, Route, Today's/Completed/Upcoming counts, notifications, and live trip banner. Today page manages full lifecycle (Start locks driver/vehicle/route via snapshot; Pause/Resume/End write timeline events). Live Trip page (/driver/trip/$tripId) renders Google Map with color-coded stops + red arrow vehicle marker, per-stop Arrive/Depart/Skip actions with boarded/missing counts, per-student attendance stored in trips.metadata.attendance, and a rolling timeline. GPS tracking uses navigator.geolocation.watchPosition (high-accuracy, ≤1 update / 8s) writing trips.live_location while status = in_progress. Navigation page deep-links Google Maps to next stop / end. QR Scanner uses getUserMedia with local scan history. SOS button appends a driver.sos event. Multi-tenant isolation is enforced by RLS + useMyDriver — a driver can never read another driver's trips/students/vehicles/routes." },
...
const API_SERVICES = [
  ...
  { name: "src/lib/portal-accounts.functions.ts", purpose: "Driver & Parent portal lifecycle. provisionPortalAccount (create auth user + role + link record), resetPortalPassword, updatePortalEmail, deletePortalAccount (revoke all sessions via auth.admin.signOut(uid,'global'), auth.admin.deleteUser, cascade user_roles, soft-delete drivers with deleted_at/deleted_by + is_active=false + user_id=null), restorePortalAccount (clear deleted_at, optionally re-provision fresh login with new email + temp password). All endpoints authorize Super Admin or the School Admin of the target school, so multi-tenant isolation holds even for admin operations. Auth context (src/lib/auth-context.tsx) additionally blocks sign-in for a driver whose row is missing, soft-deleted, or is_active=false — the session is signed out immediately with the message: 'Your account has been disabled. Please contact your school administrator.'" },
